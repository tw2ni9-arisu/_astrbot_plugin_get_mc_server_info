...
- `/查询服务器 <服务器地址>`：直接查询该地址（默认仅管理员可用；已保存地址不受此限制）。
- `/查询`：`/查询服务器` 的简称。
...
安全限制：查询、添加和重定向仅允许解析到公网 IP；回环、私有、链路本地、保留和多播地址会被拒绝。添加、删除、重命名、重定向和模板切换默认仅管理员可用，命令与 LLM Tool 执行相同权限检查。
...
默认情况下，普通成员只能查询当前会话已保存的服务器；写操作和未保存地址直连需要管理员权限。可通过下方配置显式调整。
...
| mutation_requires_admin | 写操作和模板切换是否仅限管理员（默认开启） |
| direct_query_requires_admin | 未保存地址直连查询是否仅限管理员（默认开启） |
