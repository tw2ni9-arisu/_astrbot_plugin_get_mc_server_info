...
- 重命名服务器：`#重命名服务器 <旧名称> <新名称>` 修改当前会话内服务器名称。
...
- 会话内重名检测：添加/重命名时若名称重复，会自动追加 `(<序号>)` 后缀并提示。
- 关键词简称：支持 `#添加`、`#查询`、`#删除`、`#重命名`、`#列表`。
...
- `#重命名服务器 <旧名称> <新名称>` / `#重命名 <旧名称> <新名称>`：
  - 匹配当前会话内目标服务器后执行重命名；
  - 若不存在旧名称服务器会返回提示。
- 重名检测（会话隔离）：
  - 添加或重命名时，若目标名称在当前会话内已存在，则自动追加后缀 `(<序号>)`；
  - 例如已有 `示例`，再添加/重命名为 `示例` 时会自动变为 `示例(1)`；
  - 原有服务器名称不变，并返回“名称重复已自动调整”的提示。
